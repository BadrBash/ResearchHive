...
namespace ResearchHive.Implementations.Services
{
    public class CreateLecturerRequest
    {
        public record Request(string FirstName, string LastName,
            string EmailAddress, string UserName, string PhoneNumber, List<Guid> DepartmentIds) : ICommand<Guid>
        {
            [JsonIgnore]
        ...
        public record Handler : ICommandHandler<Request, Guid>
        {
            ...
            public async Task<Result<Guid>> Handle(Request request, CancellationToken cancellationToken)
            {
                ...
                if (userResult.Succeeded)
                {
                    ...
                    var lecturerResult = await _lecturerRepository.AddAsync(lecturer);
                    var saveChangesResult = await _lecturerRepository.SaveChangesAsync(cancellationToken);
                    if (saveChangesResult != 0)
                    {
                        return await Result<Guid>.SuccessAsync(lecturerResult.Id, ResponseMessage.CreationSuccesful + $" {staffNumber}");
                    }
