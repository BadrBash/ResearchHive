...
public class CompleteSubmission
{
    ...
    public record Handler : IQueryHandler<Request, Guid>
    {
        ...
        public async Task<Result<Guid>> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            var updatedproj = Project?.CompleteSubmission();
            if (updatedproj.IsComplete)
            {
                ...
                if (saveChangesResult == 1) return await Result<Guid>.SuccessAsync(updateRequest.Id, $" {ResponseMessage.UpdateSuccessful} for Project with id: {request.Id} successfully completed - awaiting approval");
                return await Result<Guid>.FailAsync(ResponseMessage.OperationFailed);
            }
            return await Result<Guid>.FailAsync(ResponseMessage.OperationFailed + @"Previous Stage: 'In Progress' not achieved yet");
        }
    }
}
