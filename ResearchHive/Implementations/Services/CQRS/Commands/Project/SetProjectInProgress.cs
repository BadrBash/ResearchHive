...
public class SetProjectInProgress
{
    ...
    public record Handler : IQueryHandler<Request, Guid>
    {
        ...
        public async Task<Result<Guid>> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            var updatedproj =  Project?.SetInProgress();
            ...
            if (saveChangesResult == 1) return await Result<Guid>.SuccessAsync(updateRequest.Id, $" {ResponseMessage.UpdateSuccessful} for Project with id: {request.Id}");
