...
public class GetCompletedProjects
{
    ...
    public record Request(Guid studentId) : IQuery<IEnumerable<ProjectDTO>>
    {
        [JsonIgnore]
        public Guid StudentId { get; set; }
    }
    ...
    public record Handler : IQueryHandler<Request, IEnumerable<ProjectDTO>>
    {
        ...
        {
            var projects = await _projectRepository.GetAsync(proj => proj.IsComplete && !proj.IsDeleted);
