...
public class GetProjectsBySubmissionWindowId
{
    ...
    public record Handler : IQueryHandler<Request, IEnumerable<ProjectDTO>>
    {
        ...
        {
            var projects = await _projectRepository.GetAsync(proj => proj.ProjectSubmissionWindowId == request.SubmissionWindowId && !proj.IsDeleted);
