...
namespace ResearchHive.Implementations.Services.CQRS.Queries.Project
{
    public class GetApprovedProjects
    {
        ...
        public record Request() : IQuery<IEnumerable<ProjectDTO>>
        {
        ...
        public record Handler : IQueryHandler<Request, IEnumerable<ProjectDTO>>
        {
            ...
            {
                var projects = await _projectRepository.GetAsync(proj => proj.IsApproved && !proj.IsDeleted);
                ...
                var projectsDTO = projects.Select(proj => new ProjectDTO
                {
                    ...
                    IsComplete = "Project Is Complete",
