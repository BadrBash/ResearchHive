...
public class GetApprovedResearches
{
    ...
    public record Handler : IQueryHandler<Request, IEnumerable<ResearchDTO>>
    {
        ...
        {
            var Researchs = await _ResearchRepository.GetAsync(res => res.IsApproved && !res.IsDeleted);
            if (Researchs.Count() <= 0) return await Result<IEnumerable<ResearchDTO>>.FailAsync("Researches' retrieval returned empty data");
