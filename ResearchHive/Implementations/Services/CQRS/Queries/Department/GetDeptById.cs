...
public class GetDeptById
{
    ...
    public record Handler : IQueryHandler<Request, DepartmentDto>
    {
        ...
        {
            var dept = await _departmentRepository.GetAsync(dept => dept.Id == request.Id && dept.IsDeleted == false);
            if (dept is null) return await Result<DepartmentDto>.FailAsync($"department with Id: {request.Id} not found");
            ...
            return await Result<DepartmentDto>.SuccessAsync(departmentReturned, $"department with Id: {request.Id} successfully retrieved");
